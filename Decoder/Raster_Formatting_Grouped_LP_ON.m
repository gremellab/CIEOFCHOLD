function [] = Raster_Formatting_Grouped_LP_ON(Session_Directory, Raster_Destination)
...
for file = 1:length(SessionFiles)
    ...
    %destdirectory = ['I:\Christian\Code_Exp35_CIE_Hold_Eth\Code\Encoding\Raster_Format_Data\Cohort_Grouped_LP_ON_Sig\CIE\']
   % mkdir(destdirectory);   %create the directory
    ...
    sig_idx = unique([find(~cellfun(@isempty,{Data.Session.Events.LPON.PETH_stats.Up_Mod_Window}))...
        find(~cellfun(@isempty,{Data.Session.Events.LPON.PETH_stats.Down_Mod_Window}))]);
    for neuron = 1:size(Data.Session.Events.LPON.PETH_stats,2) %sig_idx
        %raster_data = Data.Session.Events.LPON.PETH_data(neuron).roll_bincount(:,8001:15001); % from -2 to 5 perievent
        raster_data = Data.Session.Events.LPON.PETH_data(neuron).bincount(:,8001:15001); % from -2 to 5 perievent
